 ...
 * @returns {any}
 */
const pickIntersection = (source, meta) => {
	...
	if (typeof source === 'object') {
		...
		return meta.children.reduce(
			(result, child) => ({
				...result,
				...pickValue(source, child)
			}),
			initial
		);
	}

	return pickValidValue(source, meta);
};
...
const pickObject = (value, meta) => {
	...
	meta.optional.forEach((child, key) => {
		if (child.def && value[key] === undefined) {
			...
			// Check for default value getter
			if (typeof child.def === 'function') {
