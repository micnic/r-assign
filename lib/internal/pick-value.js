 ...
 */
const pickObject = (value, shape) =>
	forMap(shape, (type, key) => {
		...
		if (meta.classification === 'optional') {
			...
			if (meta.type(value[key])) {
				return pickValue(value[key], meta.type);
			}
		} else {
			return pickValue(value[key], type);
		}
...
const pickTuple = (value, tuple) => {
	...
	return tuple.slice(0, value.length).map((type, index) => {
		return pickValue(value[index], type);
	});
};
