...
/**
 * @typedef {import('./index.js').LSTGM} LSTGM
 */
...
/**
 * @typedef {import('./index.js').TLTGM} TLTGM
 */
 ...
 * @returns {asserts primitive is T}
 */
export const assertPrimitive = (primitive, set, message) => {

	/** @type {PT[]} */
	const accepted = set;

	if (!accepted.includes(primitive)) {
		throw TypeError(message ?? 'Invalid type provided');
	}
};
...
* @param {ARTGM | TTGM} meta
* @returns {string}
*/
const getInputAnnotation = (meta) => {

	// Check for rest type
	if (meta.classification === 'array') {
