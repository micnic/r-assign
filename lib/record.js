...
/**
 * @typedef {import('./internal/index.js').OTTGM} OTTGM
 */
 ...
 * @template {Shape} S
 * @type {{
 *	(...args: [V]): TG<Record<string, InferTG<V>>>;
 *	(...args: [V, S]): TG<Record<string, InferTG<V>> & InferS<S>>;
 *	(...args: [K, V]): TG<Record<InferTG<K>, InferTG<V>>>;
 *	(...args: [K, V, S]): TG<Record<InferTG<K>, InferTG<V>> & InferS<S>>;
 * }}
...
export const isRecordOf = (...args) => {
	...
	/** @type {TG<Record<InferTG<K>, InferTG<V>>>} */
	const check = (value) => {
