...
const canReduceTemplate = (template) =>
	(hasOneElement(template) && template[0] === isString) ||
...
const reducePart = (result, part) => {
	...
	// Assert allowed type classification
	...
	// Check for literals type
	if (meta.classification === 'literals') {
		return result.flatMap((string) =>
			meta.literals.map((literal) => `${string}${literal}`)
		);
	}

	// Check for boolean primitive
	if (meta.classification === 'primitive') {

		// Assert for boolean primitive
		assertPrimitive(meta.primitive, ['boolean']);
	...
	return result.flatMap((string) =>
		meta.union.flatMap((type) => reducePart([string], type))
	);
};
...
const reduceTemplate = (template) => {
	...
	if (hasNoElements(template)) {
		return isLiteral('');
	}

	// Check for only one string type guard
	if (hasOneElement(template) && template[0] === isString) {
		return isString;
	}
	...
	if (parts.length > 0) {
		...
		if (prev) {
			if (typeof part === 'string') {
				...
				if (typeof prev === 'string' || prev !== isString) {
					parts.push(part);
				}
			} else {
				parts.push(part);
