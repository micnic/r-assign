	...
	switch (meta.classification) {
		...
		case 'literals': {
			...
			if (hasAtLeastTwoElements(literals)) {
				...
				const child = getTypeGuardMeta(isLiteralOf(literals));

				// Assert for literals type
				assertClassification(child.classification, ['literals']);
			...
			if (hasOneElement(literals)) {
				...
				const child = getTypeGuardMeta(isLiteral(literals[0]));

				// Assert for literal type
				assertClassification(child.classification, ['literal']);
