...
/**
 * Check for optional type with default value
 * @param {TypeGuardMeta} meta
 * @returns {boolean}
 */
const hasOptionalWithDefault = (meta) =>
	meta.classification === 'optional' && meta.def !== undefined;
...
export const isTupleOf = (types) => {
	...
	const children = tuple.map(getTypeGuardMeta);

	// Check for only one rest element in tuple to return an array type
	if (hasOneElement(children)) {
		...
		if (child.classification === 'rest') {
			...
			/** @type {TypeGuard} */
			const check = isArrayOf(child.type);
	...
	// Check for optional type with default value
	if (children.some(hasOptionalWithDefault)) {
		throw TypeError('Tuple optional type cannot have default values');
	}
